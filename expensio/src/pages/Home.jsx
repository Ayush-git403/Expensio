...
const CATEGORY_COLORS = {
  Food: '#e07820', Transport: '#1a6fad', Housing: '#3a6640',
  Health: '#e05050', Entertainment: '#7040a0', Shopping: '#c04830',
  Education: '#0f6e56', Travel: '#ba7517', Utilities: '#4a7a8a', Other: '#888888'
};
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.bg, transition: 'background 0.3s' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{
              ...
              fontSize: 13, marginTop: 6, fontWeight: 300
            }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{
              ...
              minWidth: 56, textAlign: 'center', letterSpacing: '-0.5px'
            }}>
              {year}
            </span>
            <button onClick={() => setYear(y => y + 1)} style={{
        ...
        {/* ── Summary Cards ── */}
        <div style={{
          display: 'grid', gridTemplateColumns: summaryGridCols,
          gap: isMobile ? 10 : 14, marginBottom: isMobile ? 24 : 28
        }}>
          {[
            { label: 'TOTAL SPENT',    value: `₹${totalYear.toLocaleString('en-IN')}`, sub: `in ${year}`,          color: theme.textPrimary },
            { label: 'ACTIVE MONTHS',  value: activeMonths,                             sub: 'with expenses',      color: theme.highlight },
            { label: 'MONTHLY AVG',    value: activeMonths ? `₹${Math.round(totalYear / activeMonths).toLocaleString('en-IN')}` : '₹0', sub: 'per month', color: theme.textPrimary },
            { label: 'BUSIEST MONTH',  value: Object.keys(monthlyTotals).length ? MONTHS[Number(Object.entries(monthlyTotals).sort((a,b)=>b[1]-a[1])[0]?.[0])]?.slice(0,3) : '—', sub: 'highest spending', color: theme.textPrimary },
          ].map(c => (
            <div key={c.label} style={{ ...cardStyle, padding: isMobile ? '16px 14px' : '22px 20px' }}>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 10, color: theme.textMuted, letterSpacing: '0.08em', fontWeight: 500, marginBottom: 8 }}>{c.label}</div>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: isMobile ? 20 : 26, color: c.color, fontWeight: 700, letterSpacing: '-0.5px' }}>{c.value}</div>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 11, color: theme.textSubtle, marginTop: 5, fontWeight: 300 }}>{c.sub}</div>
            </div>
          ))}
        </div>
        ...
        {!loading && (
          <>
            ...
            <div style={{
              ...
              {/* Bar Chart */}
              <div style={{ ...cardStyle, padding: isMobile ? '16px 14px' : '22px 20px' }}>
                <div style={{
                ...
                {totalYear > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <XAxis
                        dataKey="month"
                        tick={{ fill: theme.textMuted, fontSize: 10, fontFamily: 'DM Sans, sans-serif' }}
                      ...
                      <div key={exp._id} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{
                            ...
                            background: i === 0 ? (dark ? '#fff' : '#000') : theme.cardBorder,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 10, fontWeight: 700, flexShrink: 0,
                            color: i === 0 ? (dark ? '#000' : '#fff') : theme.textMuted,
                            fontFamily: 'DM Sans, sans-serif'
                          }}>
                            {i + 1}
                          </div>
                        ...
                        <div style={{
                          ...
                          color: theme.monthCardText,
                          letterSpacing: '-0.3px', flexShrink: 0, marginLeft: 8
                        }}>
            ...
            {MONTHS.map((name, i) => {
              ...
              return (
                <div key={i} onClick={() => setSelectedMonth(i)}
                  ...
                  onMouseOut={e => {
                    e.currentTarget.style.borderColor = isCurrent ? theme.accent : theme.monthCardBorder;
                  ...
                  {isCurrent && (
                    <div style={{
                      ...
                      fontSize: 9, color: dark ? '#000000' : '#ffffff',
                      background: theme.accent, padding: '3px 9px',
                  ...
                  <div style={{
                    ...
                    color: theme.monthCardText, fontWeight: 700,
                  ...
                  <div style={{
                    ...
                    letterSpacing: '-0.5px', opacity: total ? 1 : 0.35
                  }}>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      background: barWidth > 75 ? theme.highlight : theme.monthCardText,
                  ...
                  {!isMobile && (
                    <div style={{
                      fontFamily: 'DM Sans, sans-serif', fontSize: 11,
                      color: theme.monthCardMuted, marginTop: 9,
                      display: 'flex', justifyContent: 'space-between', fontWeight: 300
                    }}>
                      <span>{total ? `${barWidth}% of peak` : 'No expenses'}</span>
                      {total && <span style={{ color: theme.monthCardText, fontWeight: 600 }}>View →</span>}
                    </div>
                  )}
                </div>
              );
